<?php

namespace Sylphian\Verify\Pub\Controller;

use Sylphian\Verify\Model\GameServer;
use XF\Mvc\Controller;
use XF\Mvc\Reply\View;

class Verify extends Controller
{
    public function actionIndex(): View
    {
        $servers = [
            'mc-0' => new GameServer(
                title: 'Minecraft - Lobby World',
                motd: 'Lobby server for a network',
                game: 'Minecraft',
                host: 'mc.example.com',
                port: 25565,
                max_players: 50,
                players: 12,
            ),
            'mc-1' => new GameServer(
                title: 'Minecraft - Survival World',
                motd: 'Show your creativity in our Minecraft server.',
                game: 'Minecraft',
                host: 'mc.example.com',
                port: 25566,
                max_players: 200,
                players: 145,
                icon: 'https://api.mcstatus.io/v2/icon/mc.example.com'
            ),
            'mc-2' => new GameServer(
                title: 'Minecraft - Creative World',
                motd: 'Show your creativity in our Minecraft server.',
                game: 'Minecraft',
                host: 'mc.example.com',
                port: 25567,
                max_players: 100,
                players: 42,
                favicon: 'data:image/x-icon;base64,'
            )
        ];

        $viewParams = [
            'servers' => array_map(fn(GameServer $s) => $s->toArray(), $servers),
            'statuses' => array_map(fn(GameServer $s) => $s->toArray(), $servers)
        ];

        return $this->view('Sylphian\Verify:Verify', 'sylphian_verify_game_servers', $viewParams);
    }
}